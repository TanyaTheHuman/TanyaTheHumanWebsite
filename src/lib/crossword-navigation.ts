...
export function getFirstEmptyCellInNextWord(
  ...
  const currentWord = getWordContaining(
    data,
    currentRow,
    currentCol,
    wordDirection,
...
export function getFirstEmptyCellInPreviousWord(
  ...
  for (let i = 1; i <= sortedWords.length; i++) {
    const prevIndex =
      (currentIndex - i + sortedWords.length) % sortedWords.length;
...
export function getNextWordCell(
  ...
  const words = wordDirection === "across" ? data.acrossWords : data.downWords;
  const currentWordId =
    wordDirection === "across" ? cell.acrossWordId : cell.downWordId;
  ...
  if (isAtEnd || isAtStart) {
    ...
    const otherDirection: WordDirection =
      wordDirection === "across" ? "down" : "across";
    const otherWords =
      otherDirection === "across" ? data.acrossWords : data.downWords;
    const sortedOtherWords = [...otherWords].sort(
    ...
    const targetWord = isAtEnd
      ? sortedOtherWords[0]
      : sortedOtherWords[sortedOtherWords.length - 1];
